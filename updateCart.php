...
//	$cartId     = $_POST['hidCartId']; //เก็บรหัสตะกร้า ซึ่งเป็นแบบอาร์เรย์
                    ...
                    $ct_qty = $row['ct_qty'];  
                        ...
                        if ($newQty < 1) { //คือ ลูกค้าใส่ 0 มา หมายถึงไม่มีสินค้า
                           ...
                           // ถ้าจำนวนสินค้าที่ลูกค้าเปลี่ยนน้อยกว่า 1 ให้ลบสินค้าออกจากตะกร้า (คือ ลูกค้าใส่ 0 มา หมายถึงไม่มีสินค้า)
                            ...
                            // ถ้าใส่ 0 จนไม่มี สนค้าเหลืออยู่ในตาราง cart_item ให้ลบ $sid ไปเลยในตาราง cart
                            ...
                            $qty_stock = $row['pd_qty']; //ค่าในสต๊อก
                                    ...
                                    if ($newQty > $ct_qty) {  //หยิบเพิ่ม
                                         $X = $newQty; //(ค่าใหม่)
                                         $X -= $ct_qty;  //(ค่าใหม่ - ค่าเดิม = Y) ; Y คือ ส่วนต่าง
                                         ...
                                         $Y = $ct_qty;  //(ค่าเดิม)
                                         $Y -= $newQty ;  //(ค่าเดิม - ค่าใหม่ = X) ; X คือ ส่วนต่าง
...
//                            $result_cart_price  = mysql_query($sql_cart_price) or die(mysql_error()); 
